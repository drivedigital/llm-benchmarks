    ...
    <div className="flex max-h-[560px] flex-col gap-2 overflow-y-auto pr-1">
      {runs.length === 0 && (
        <p className="py-8 text-center text-sm text-slate-500">
          No runs yet. Results appear only after you start a real API test.
        </p>
      )}
      {runs.map((run) => {
        const { model, test } = run;
        return (
          <button
            key={run.id}
          ...
          >
            <div className="shrink-0">
              ...
              {run.status === "cancelled" && (
                <Ban className="h-4.5 w-4.5 text-slate-400" />
              )}
            </div>
            ...
            <div className="min-w-0 flex-1">
              ...
              {run.status === "running" ? (
                ...
                <p className="mt-0.5 truncate text-[12px] text-rose-400/80">
                  {run.error}
                </p>
              ) : (
