  ...
  return (
    <Modal open={open} onClose={onClose} title="Server & Roster Settings" icon={<Plug className="h-4 w-4" />} wide>
      <div className="space-y-8">
        <section>
          <h3 className="mb-2 text-sm font-semibold text-slate-200">Jan API Server</h3>
          <p className="mb-3 text-xs text-slate-500">
            Point this dashboard at any Jan.ai OpenAI-compatible REST endpoint. Browser-side
            requests to a local server may be blocked by CORS \u2014 the dashboard will keep
            running in simulated mode if the probe fails.
          </p>
                ...
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[11px] text-slate-500">
                      {m.engine} \u00b7 {m.paramSize}
                      {m.visionCapable ? " \u00b7 vision" : ""}
                      {m.custom ? " \u00b7 custom" : ""}
                    </p>
                  </div>
                </div>
        ...
        <section>
          ...
          <p className="mb-3 text-xs text-slate-500">
            Add any model identifier being served by your Jan instance (or a hypothetical one)
            and it will join the live rotation immediately with an estimated performance
            profile.
          </p>
